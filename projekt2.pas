...
begin
 clrscr;
 write('Namen der Datei [Laufwerk\Verz\Name.Erw] : ');
 readln(s);
...
{ clrscr;
 writeln('Haben sie ihre Daten gespeicher ?(Anzeigen/Speichern/Beenden)');
 repeat
  taste:=readkey;
  taste:=Upcase(Taste);
  if Taste='A' then Aschuanz;
  if taste='S' then chuspei;
 until taste='B';}
